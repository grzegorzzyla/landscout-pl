...
import { pythonExe, startClaudeJob, findRunningJob } from '../../server/runner';
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const prompt =
      ...
      `2. \`${pythonExe()} scripts/manage_listing.py get --id ${id}\` — weź description, pola ORAZ notes[]. ` +
      ...
      `   - zapamiętaj stary score; \`${pythonExe()} scripts/manage_listing.py set --id ${id} --field score --value <nowy_score>\`\n` +
      `   - zapisz plik z treścią "**Ocena: <score>/100 — <werdykt>** (po notatce, <data>)\\n\\n<reason>" i ` +
      `\`${pythonExe()} scripts/manage_listing.py set-section --id ${id} --header "Ocena dopasowania" --text-file <plik>\`\n` +
      `   - dopisz do historii: \`${pythonExe()} scripts/manage_listing.py log --id ${id} --text "ponowna ocena (notatka): score <nowy> (było <stary>)"\`\n` +
